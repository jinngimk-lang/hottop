...
Hottop's unresolved Mandarin quality ceiling is still a real operator-provisioned Qwen3-TTS 1.7B CustomVoice same-line A/B. Public serving results are useful for planning that benchmark, but they can be misread if serving topology is not bound as provenance.
...
A serving-topology improvement is not a model-quality improvement. Future Qwen3-TTS acceleration evidence must keep at least these dimensions separate:
...
6. **throughput/latency** — QPS, TTFA and overload/failure behavior;
7. **audio quality** — same-line Mandarin intelligibility, delivery/naturalness, non-empty/finite/non-silent serialized PCM and slot-fit duration;
8. **publication rights** and no hidden paid/network/model-fetch dependency.

Do not compare a single-instance Hottop result against a multi-replica/MPS public number as though they were the same system. Do not credit dispatch/process-topology gains to Qwen model quality or to an inference kernel unless the A/B isolates that variable.
...
This adds one provenance dimension to the existing TTS serving-evidence contract: **serving topology is part of the measured system**. It does not weaken the existing 1.7B capability gate, final PCM integrity gates, publication-rights review, eSpeak fallback or zero-paid/no-auto-download policy.
