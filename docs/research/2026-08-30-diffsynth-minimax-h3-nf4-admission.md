...
The quantized repository's `apache-2.0` metadata is **not sufficient by itself** to erase obligations inherited from the MiniMax-H3 base model. Independent ecosystem mirrors explicitly describe MiniMax-H3 as carrying a MiniMax H3 Community License with commercial/revenue, territory, attribution and cross-model-improvement restrictions.
...
Before any operator benchmark is promoted beyond research, re-check the **official MiniMax-H3 license text at the exact model revision** and reconcile it with the derivative NF4 model card. A permissive derivative metadata field never overrides a more restrictive upstream/base-model grant.
...
- exact official base-model/derivative rights are compatible with the intended use and geography;
