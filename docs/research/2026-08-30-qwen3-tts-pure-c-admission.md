...
The structural gate additionally follows the safetensors format's own tensor-layout semantics. For each supported dtype, `data_offsets[1] - data_offsets[0]` must equal the byte size implied by `dtype × shape`; unknown dtypes and sub-byte shapes whose total bit count is not byte-aligned fail closed. Once every tensor descriptor is individually valid, the sorted offset spans must cover the complete data buffer contiguously, with no gap, overlap or trailing unindexed bytes. These checks remain metadata-only and dependency-free: they do not materialize tensors or weaken the existing stable-snapshot/streaming-hash boundary.

The gate also follows the upstream safetensors **100,000,000-byte maximum header size**. A declared JSON header above that ceiling is rejected before Hottop allocates/reads the header. The complete file is still SHA-256-bound through fixed-size streaming chunks, so a rejected oversized artifact remains auditable without making the read-only preflight itself a memory-amplification path.
