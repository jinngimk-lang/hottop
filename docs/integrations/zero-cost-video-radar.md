...
| `WeChatCV/Identity-as-Presence` | Wan2.2-TI2V-5B appearance + voice identity mechanism for joint audio-video personalization | Exact source `e905e6ef67df77aa4b969a2ddc02c488505f2eea` has no root `LICENSE`; only single-subject inference/weights are marked released while multi-subject remains TODO. The public Identity-as-Presence weights declare Apache-2.0, Wan2.2-TI2V-5B declares Apache-2.0, but the required MMAudio checkpoint is CC BY-NC 4.0/non-commercial. Voice/face references are separately rights-gated. Official inference uses `torchrun --nproc_per_node=8` and manually provisioned checkpoints. | **Research-only identity/voice mechanism; commercial/runtime gate unresolved, no code/weights ingestion** |
...
| `WildActor/WildActor` | Wan2.2-5B multi-reference full-body human identity candidate | Exact source `c858c2100ed14b32c36883e0570948f4c09e0d28` exposes Wan2.2 inference code but the inspected root tree has no license file. Human-only/data-pipeline scope also requires separate dataset/reference-rights review. | **Gated research candidate pending exact license/weights review** |
...
- `WeChatCV/Identity-as-Presence@e905e6ef67df77aa4b969a2ddc02c488505f2eea` is especially relevant because it treats facial appearance and vocal timbre as identity-bearing controls for joint audio-video generation on Wan2.2-TI2V-5B. It does **not** clear Hottop's admission gate: the exact GitHub root has no `LICENSE`; README marks only single-subject inference/weights released while multi-subject remains TODO; official inference launches eight local processes and requires manually provisioned Identity-as-Presence, Wan2.2-TI2V-5B and MMAudio checkpoints; MMAudio's published checkpoint is CC BY-NC 4.0/non-commercial. Keep it research-only. If its identity injection mechanism later becomes permissively licensed and separable from non-commercial audio weights, reassess the visual-identity mechanism independently rather than inheriting the full joint A/V stack.
