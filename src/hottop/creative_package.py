...
def build_creative_package(package: CreativePackageInput) -> CreativePackageResult:
    ...
    diagnostics = [CreativeOptionDiagnostic(index=index, label=option.label, total=option.review.total, passes=option.review.passes, expression_form=option.concept.strategy.expression_form, bridge=option.concept.strategy.bridge) for index, option in enumerate(package.options)]
    eligible = [(index, option) for index, option in enumerate(package.options) if option.review.passes]
    if not eligible:
    ...
    return CreativePackageResult(selected_index=selected_index, selected_concept=selected.concept, selected_review=selected.review, selected_render=build_creative_render_request(selected.concept), option_diagnostics=diagnostics, references=package.references)
