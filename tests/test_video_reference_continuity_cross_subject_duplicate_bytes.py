...
def test_continuity_benchmark_rejects_cross_subject_duplicate_shot_bytes(
    tmp_path: Path,
) -> None:
    reference_paths = {
    ...
    reference_paths["hero-a"].write_bytes(b"generated-original-hero-a")
    reference_paths["hero-b"].write_bytes(b"generated-original-hero-b")

    shot_paths = [tmp_path / "shot-001.mp4", tmp_path / "shot-002.mp4"]
    for path in shot_paths:
        path.write_bytes(b"same-generated-shot-for-two-distinct-subjects")

    shot_sha = hashlib.sha256(shot_paths[0].read_bytes()).hexdigest()
    ...
    evidence = ReferenceContinuityBenchmark(
        ...
        subjects=[
            ...
            for subject_id, reference_path in reference_paths.items()
        ],
    )

    with pytest.raises(ValueError, match="distinct subjects require distinct subject-bearing shot artifacts"):
        verify_reference_continuity_artifacts(
            evidence,
            manifest,
            reference_paths,
