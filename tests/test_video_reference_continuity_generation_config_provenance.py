...
def test_continuity_benchmark_rejects_mixed_generation_config_bytes(tmp_path: Path) -> None:
    ...
    config_hashes = [hashlib.sha256(value).hexdigest() for value in (b"config-a", b"config-b")]
    manifest = VideoArtifactManifest(
        ...
        shots=[
            VideoShotArtifact(
                ...
                generation_config_sha256=config_hashes[index - 1],
                generation_config_size_bytes=8,
            )
