...
def test_inspect_video_quality_rejects_partial_terminal_frame_bytes(tmp_path: Path):
    path = tmp_path / "partial-tail.mp4"
    path.write_bytes(b"video")
    ...
    def runner(args, **kwargs):
        ...
        if "-sseof" in args:
            return SimpleNamespace(returncode=0, stdout=b"\x00", stderr=b"")
        return SimpleNamespace(
            returncode=0,
            stdout=bytes([0] * (96 * 54)) + bytes([10] * (96 * 54)),
            stderr=b"",
...
def test_inspect_video_quality_rejects_partial_motion_sample_payload(tmp_path: Path):
    path = tmp_path / "partial-motion-sample.mp4"
    path.write_bytes(b"video")
    frame_size = 96 * 54
    ...
    assert report.pass_ is False
    assert "motion sample payload incomplete" in report.reasons
