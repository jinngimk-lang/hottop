...
def test_continuity_benchmark_rejects_duplicate_subject_shot_bytes(tmp_path: Path) -> None:
    reference_path = tmp_path / "reference.ppm"
    reference_path.write_bytes(b"generated-original-reference")

    shot_paths = [
        tmp_path / "shot-001.mp4",
        tmp_path / "shot-002.mp4",
        tmp_path / "shot-003.mp4",
    ]
    shot_paths[0].write_bytes(b"duplicate-generated-shot")
    shot_paths[1].write_bytes(b"duplicate-generated-shot")
    shot_paths[2].write_bytes(b"distinct-generated-shot")
    ...
    plan = VideoProductionPlan(
        ...
        shots=[
            VideoShot(
                ...
                intent=f"show requested action {index}",
                continuity_instruction="preserve hero identity",
                generation_prompt=f"hero performs action {index}",
                negative_prompt="identity drift or frozen duplicate motion",
    ...
    evidence = ReferenceContinuityBenchmark(
        ...
        subjects=[
            SubjectContinuityEvidence(
                ...
                shot_sha256s=[shot_hashes[0], shot_hashes[2]],
    ...
    with pytest.raises(ValueError, match="unique subject-bearing shot artifacts"):
        verify_reference_continuity_artifacts(
            evidence,
            manifest,
            {"hero": reference_path},
