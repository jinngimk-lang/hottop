...
def _request() -> CreativeRenderRequest:
    return CreativeRenderRequest(
        topic_id="artifact-content-integrity",
        topic_title="artifact content integrity",
        ...
        punchlines=["artifact bytes must stay bound"],
...
def _config() -> VideoProductionConfig:
    return VideoProductionConfig.model_validate(
        {
            "name": "zero-cost-content-integrity",
...
def test_execute_rejects_artifact_manifest_for_different_video_bytes(monkeypatch, tmp_path: Path):
    actual_bytes = b"actual-fresh-video"
    claimed_bytes = b"different-video"
...
def test_execute_rejects_zero_cost_artifact_without_byte_identity(monkeypatch, tmp_path: Path):
    actual_bytes = b"actual-fresh-video"
