...
def test_pure_c_preflight_binds_operator_provisioned_model_tree_without_execution(tmp_path: Path) -> None:
    ...
    assert result.checkpoint_model_type == "custom_voice"
    assert result.checkpoint_model_size == "1b7"
...
def test_pure_c_preflight_rejects_non_custom_voice_checkpoint(tmp_path: Path) -> None:
    ...
    assert result.ready is False
    assert result.checkpoint_model_type == "base"
    assert "custom_voice" in " ".join(result.blockers).lower()


def test_pure_c_preflight_rejects_non_1b7_custom_voice_checkpoint(tmp_path: Path) -> None:
    executable = _write_executable(tmp_path)
    model_dir = _write_model_dir(tmp_path)
    (model_dir / "config.json").write_text(
        json.dumps(
    ...
    assert result.ready is False
    assert result.checkpoint_model_size == "0b6"
    assert "1b7" in " ".join(result.blockers).lower()
...
def test_model_hub_cli_exposes_read_only_pure_c_preflight(tmp_path: Path) -> None:
    ...
    assert '"checkpoint_model_type": "custom_voice"' in result.stdout
    assert '"checkpoint_model_size": "1b7"' in result.stdout
