...
def test_lightx2v_fails_closed_before_gpu_when_config_json_has_duplicate_object_keys(
    tmp_path: Path,
):
    root = tmp_path / "LightX2V"
    (root / "lightx2v").mkdir(parents=True)
    ...
    config_json.parent.mkdir(parents=True)
    config_json.write_text(
        '{"guidance_scale": 3.0, "guidance_scale": 7.5}\n',
        encoding="utf-8",
    )
    model_path = tmp_path / "Wan2.2-I2V-A14B"
    model_path.mkdir()
    ...
    def gpu_probe():
        raise AssertionError("GPU probe must not start when config JSON keys are ambiguous")
