...
PR #136 (`docs: review ID-V2V continuity candidate`) passed exact-head CI **#1909** at `768ff45eb2471712bb121f91a8a64de3512eef63`, was squash-merged to `main` as `107d3cfbe89d634769b9dfc0c562dd1fe10401f8`, and post-merge CI **#1910** passed in Python 3.11 and 3.12. The admission is documentation/research only: no provider route, model download, HF login, FFmpeg install, GPU provisioning or readiness claim was added.

The ID-V2V takeaway strengthens existing canonical doctrine rather than changing it: when a rights-safe source performance already exists, a motion-preserving V2V restylization route may be a better recovery strategy than regenerating motion from a still; identity fidelity and motion/performance fidelity still require separate output-side evidence. `PROJECT.md` already carries the relevant image-first recovery and identity-vs-motion rules, so no duplicate doctrine was added.
...
- `Ashokbtech87/ID-V2Vp@f1d90a51a7bfff7accbec98813718cd8babb81df` is now durably reviewed as a motion-preserving restylization benchmark candidate. Its useful niche is a rights-safe source performance plus stylized keyframes; it does not replace still-reference LightX2V/Wan2.2, and its heavy/gated setup remains operator-only.
...
- LightX2V/Wan2.2 has no newly measured Hottop continuity/runtime gain that justifies freshness-only repinning. Recent visible activity remains model requests/maintenance rather than a measured improvement to Hottop's tested Wan2.2 I2V subset; community HIGH/LOW expert-LoRA routing remains a correctness caution rather than a reason to change the tested route.
...
4. Re-evaluate ID-V2V only after an operator has already provisioned its exact source/checkpoint/runtime stack and supplies a rights-safe source performance video plus stylized keyframes; compare identity, source-motion/performance fidelity and style/keyframe adherence before any executable route.
