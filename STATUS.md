...
Live recovery on 2026-08-28 observed `main@753b8d7af6c147720496ed3c90deeba67e48fd2d`; CI **#1969** passed and no open PR existed at recovery. The previous status pointer was stale. This head includes the gated MultiShotMaster continuity admission while preserving the guaranteed Production v0.2 baseline.
...
TFM (`pzrain/TFM@ab791c05748019d90742e7fcebf599daacdbd824`) is now a **research-only motion-fidelity candidate**. The source repository is Apache-2.0 and upstream documents single-A100-40GB inference on Wan2.1-T2V-14B plus a released LoRA, but the reviewed repository surface does not independently establish a LoRA-specific license. TFM targets temporal/motion coherence, not subject identity. It has no Hottop executable route, must not trigger the Google Drive LoRA or Wan base-model downloads unattended, and may be reconsidered only with exact checkpoint rights, local operator provisioning and same-sequence requested-action evidence. See `docs/research/2026-08-28-tfm-motion-fidelity-admission.md`.

## Fresh ecosystem radar — 2026-08-28

Targeted checks remain gap-driven rather than popularity-driven.

- **TFM:** reviewed `pzrain/TFM@ab791c05748019d90742e7fcebf599daacdbd824` applies temporal-aware flow-matching constraints to Wan2.1-T2V-14B and reports no added inference mechanism beyond the trained LoRA. Source is Apache-2.0; upstream recommends a single A100 40GB for inference and 4×A100 80GB for week-scale training. The LoRA is distributed through Google Drive without an independently reviewed LoRA-specific license declaration in the inspected repository surface, so TFM remains research-only. It can inform a future **motion-fidelity** A/B but cannot stand in for identity/reference continuity.
