...
- Concurrent main work recorded #29/#32 720p byte-equality evidence. The complete #29/#31/#32/#38 evidence below narrows that interpretation.
...
- #38 final SHA: `c1353b556cb8675b94e58bb1d41624c69b4711ad1b83c690f1e81dd60b3f58df`.

Thus #29/#32/#38 match, but #31 is a real counterexample to treating the 720p route itself as universally bitwise deterministic. All five software3d shot bytes in #31 differ from #32 even though the derived plan and then-recorded package/executable/font runtime provenance are byte-identical.

This is **not currently a visual quality failure**. #31 seam evidence was intra p95 `0.933076`, max delta `4.178889`, ratio `4.478614`; #32 was `0.933903`, `4.184792`, `4.480971`; #38 again measured `0.933903`, `4.184792`, `4.480971`. All pass with margin. A decoded 90×160 grayscale comparison of #31/#32 has mean absolute difference about **0.043/255** and only about **0.31%** of sampled pixels differ by more than one level.
...
## Runtime provenance closure — verified

#31 and #32 used the same recorded packages, FFmpeg/FFprobe/eSpeak executable bytes and caption font, but different hosted workers/regions. CPU/hardware execution identity was missing from the prior `hottop.runtime-provenance.v1` capture in the 720p workflow.

PR #97 added a TDD contract for:

- `platform.machine()`;
- first-processor `/proc/cpuinfo` model name and vendor ID;
- SHA-256 of the complete `/proc/cpuinfo` bytes.

RED CI #1745 passed Ruff and failed exactly the new CPU-provenance contract (`1 failed / 500 passed`). After correcting an over-specific test-shape assertion, exact-head CI #1750 passed on Python 3.11/3.12.

Exact-head cinematic-delivery-smoke #38 then passed the full real 720p24 Odyssey chain and recorded:

- machine: `x86_64`;
- CPU model: `AMD EPYC 7763 64-Core Processor`;
- vendor: `AuthenticAMD`;
- `/proc/cpuinfo` SHA-256: `e8c8a04bfd1dcda906a9b8e1116f3db8b87b00df7e0265072c3b0083a62a37d3`;
- artifact digest: `sha256:ec3cd28eebdb41b62ef9098f63918f0052e814879ddfa14d7a1a4cb61808869f`;
- final MP4 SHA-256: `c1353b556cb8675b94e58bb1d41624c69b4711ad1b83c690f1e81dd60b3f58df`;
- seam metrics: intra p95 `0.933903`, max delta `4.184792`, max ratio `4.480971`;
- final media: H.264/yuv420p, 720×1280, 24fps, AAC, 15.0s.

This proves the CPU identity is now bound into new 720p evidence. It does **not** prove CPU differences caused #31's byte variance because historical #31/#32 CPU identities were not captured. The renderer, FFmpeg settings and quality thresholds were not changed by this closure.

FFmpeg community guidance also warns that real multithreaded encoders are not generally expected to produce byte-identical output across runs, and NumPy uses runtime CPU/SIMD dispatch. Therefore Hottop does not degrade quality or throughput merely to chase a universal byte hash; it binds runtime identity and enforces output quality/integrity instead.
...
- Official Qwen3-TTS remains `022e286b98fbec7e1e916cb940cdf532cd9f488e` in the latest checked state; there is still no operator-provisioned 1.7B same-line Mandarin A/B evidence that justifies replacing the guaranteed eSpeak-family fallback.
...
- CPU/hardware execution identity in new 720p runtime provenance.

## Immediate next actions

1. Merge PR #97 only after its latest docs-only exact-head CI is green and review threads remain clear; then verify post-merge main CI and 720p smoke because the workflow changed.
