...
- **neural-TTS waveform integrity is fail-closed before serialization:** output samples must be non-empty, finite and non-silent before any PCM/WAV creation. “Non-silent” currently means the narrow exact-all-zero rejection contract; do not replace it with a broad RMS/VAD/loudness heuristic that could reject quiet valid speech. Duration, intelligibility, delivery and final-media checks remain separate higher-level gates;
