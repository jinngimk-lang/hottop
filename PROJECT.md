...
For Git-backed LightX2V execution, source provenance also fails closed on **tracked symlinks whose resolved targets escape the checkout**. A clean Git HEAD binds the symlink record but not mutable bytes behind an external target, so such code/runtime escapes are not admissible generator identity. Tracked symlinks that resolve inside the checkout remain bound to the local execution tree and are not rejected by this rule.
...
- **speaker/voice conditioning is checkpoint-capability gated and fail-closed:** when a request selects a preset speaker, voice, instruction mode or cloning/reference mode, the exact runtime/checkpoint path must support that conditioning; silently dropping unsupported conditioning and continuing as unconditioned speech is a production failure;
...
- **2026-08-31 — LightX2V Git provenance rejects checkout-escaping tracked symlinks.** A clean HEAD binds the symlink record but not mutable bytes behind a target outside the checkout. Hottop now fails closed before generation when any tracked symlink resolves outside the operator checkout; internal tracked symlinks remain admissible when they resolve inside the bound local tree. The reviewed LightX2V `d6cf4f13d152e636ae6daac604d46531077e8670` tree contains no tracked symlinks, so this closes a concrete provenance escape without forcing a freshness-only upstream repin.
- **2026-08-29 — Unsupported neural-TTS conditioning must fail closed before synthesis.** A runtime/checkpoint may expose a generic `voice` surface while a specific checkpoint lacks preset-speaker capability. Hottop treats silent dropping of requested speaker/voice/instruction/reference conditioning as production failure. Capability validation precedes execution; repeated output-side speaker consistency remains a separate evidence gate after valid conditioning is applied. This sharpens the existing checkpoint-capability doctrine without generalizing a runtime-specific MLX-Audio Base-model bug to CustomVoice or other runtimes.
