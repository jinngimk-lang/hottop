...
- when an acceptance/delivery claim depends on media stored only in an ephemeral CI/object-store artifact, retain a **small checked-in evidence manifest** binding the exact source/config, head/run, artifact digest, final-media hashes/specs, relevant per-shot provenance and explicit claim scope. Large media does not need to live in Git, but expiring media must not be the only recoverable evidence for a durable project claim.
...
- **2026-08-26 — Durable production claims must outlive ephemeral media artifacts.** The first scoped 720×1280/24fps Odyssey delivery proof succeeded with a real Actions MP4, but the media artifact has a finite retention window. Hottop now retains a small checked-in evidence manifest binding the exact source/config, workflow head/run/artifact digest, final-media hash/specs, relevant shot provenance, measured inspection scope and claim boundary while keeping large media out of Git. This supersedes the weaker assumption that an expiring CI artifact plus a status note is sufficient long-term acceptance evidence.
