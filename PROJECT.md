...
**Perceptible motion is a delivery gate, not an aesthetic option.** Different encoded frames are insufficient if a narrative shot still reads as frozen. Deterministic software3d shots must pass the repository's measured motion/duplicate policy as well as codec/provenance checks. Anti-Polish may use deliberately obvious or awkward pan+dolly motion; cinematic profiles should use controlled camera grammar such as pan/crane/dolly/zoom. The directing treatment is style-routed, but near-static motion cannot be excused as roughness. The accepted Production v0.2 evidence and thresholds are recorded in `docs/decisions/2026-08-25-perceptible-software3d-motion.md`.
...
- **2026-08-25 — Perceptible motion becomes a deterministic delivery gate.** Production artifact inspection proved that frame-unique software3d MP4s could still read as near-frozen cards. Hottop now requires measured perceptible motion for the guaranteed deterministic baseline as well as codec/provenance validity. Anti-Polish and cinematic stories keep different directing grammar, but neither may pass with functionally frozen motion. This supersedes the weaker assumption that changing frames alone imply acceptable narrative motion.
