...
**Repeatability is contract-first, not hash-first.** For software3d and other deterministic production paths, success means the bound source/plan/provenance are valid and the accepted visual, audio, media and integrity gates reproduce. Byte-identical outputs are useful additional evidence only when actually observed under a scoped, bound runtime identity. A later different encoding/hash is not automatically a regression when the accepted production contracts still pass. Bind material runtime/hardware identity to interpret cross-run differences, and never degrade visual quality or throughput solely to chase a universal byte hash.

**Deterministic fallback must be story-explicit.** A software3d renderer may claim only story profiles it actually implements. Shot-mode rendering must derive story identity from the output workspace plan, never from an unrelated current working directory. Missing workspace plans and blank, unknown or unsupported story topics fail closed; they must never silently reuse the cow template or another historical story merely to produce a playable MP4. Distinct checked-in story proofs must produce materially distinct world/character/prop staging, not only different captions.

**Mobile-first framing includes subject scale, not only placement.** A portrait asset can satisfy top/bottom safe-area bounds while its key character or decisive prop is still too small to read on a phone. For 9:16 production evidence, inspect both principal-subject placement and readable projected scale; use backend/style-specific measured contracts rather than a universal magic number. Preserve subtitle safe areas and scene geography while correcting scale. This supersedes the weaker assumption that moving subjects into the upper canvas is sufficient mobile framing proof.

**Mobile-first subtitle quality includes line breaks, not only containment and block height.** Production evidence must inspect whether the compositor creates awkward orphan lines, especially short mixed CJK/Latin captions. Preserve full semantic copy and the existing readable font floor: prefer natural-width single-line layout when it fits, and when it is only slightly too wide, shrink the single line within that floor before allowing wrapped-caption fallback. A one-character line created solely by generic wrapping is a delivery-quality failure even if the subtitle remains inside the safe area.
...
- when the guaranteed eSpeak-NG/eSpeak backend exposes safe built-in controls, preserved dialogue metadata must be operational rather than archival-only: recurring character identity maps to a stable bounded deterministic pitch and `delivery` maps to bounded cadence around the configured base rate. This is purposeful low-fidelity differentiation, not a neural/natural-acting claim and not a universal collision-free speaker-identity guarantee;
- CosyVoice3 and Qwen3-TTS CustomVoice are reviewed operator-owned local quality/benchmark candidates;
- **neural-TTS waveform integrity is fail-closed against the audio that will actually be serialized:** model-returned samples must be non-empty and finite, then the exact int16 PCM representation destined for WAV must contain at least one non-zero sample before any WAV/temporary-file creation. Float-domain non-zero is insufficient because sub-LSB values can quantize to digital silence. “Non-silent” deliberately remains this narrow serialized-PCM exact-all-zero rejection contract; do not replace it with a broad RMS/VAD/loudness heuristic that could reject quiet valid speech. Duration, intelligibility, delivery and final-media checks remain separate higher-level gates;
- Qwen role-aware Production routing is **checkpoint-capability gated**: do not infer delivery/prosody control from the API signature alone. Current official 0.6B CustomVoice ignores `instruct`, so delivery-controlled `video-run` fails closed on `tts_model_size=0b6`; current 1.7B (`1b7`) is the admitted instruct-capable checkpoint. Standalone 0.6B basic synthesis may remain available only when no instruction is requested;
- preset-speaker/output publication rights remain separate from repository/model licensing. Apache-2.0 metadata does not by itself prove commercial speaker/timbre clearance; retain operator review before external commercial publication;
...
**Mandarin/CJK subtitle readability is a fail-closed delivery boundary.** CJK captions must resolve a real locally available CJK-capable font before MoviePy composition. `HOTTOP_CAPTION_FONT` may point to an operator-provided local font; known installed system CJK fonts may be auto-detected. If no suitable local font exists, composition fails instead of rendering tofu/replacement glyphs. Normal `video-run` never auto-installs or vendors fonts; CI/production-smoke may explicitly provision a reviewed system font package. Readability also includes line-break quality: short mixed-script captions that can fit within the readable font floor should not be forced into one-character orphan lines by generic wrapping.
...
- **2026-08-27 — Neural-TTS silence is judged on serialized PCM, not only the model-returned float waveform.** A non-zero sub-LSB waveform can quantize entirely to int16 zero and produce a digitally silent WAV while passing a float-domain `sample != 0` check. PR #115 established a shared Qwen3-TTS/CosyVoice3 contract that quantizes first, then rejects an all-zero PCM representation before any WAV/temporary-file creation. This supersedes the weaker interpretation of the earlier exact-zero rule while deliberately retaining the no-RMS/no-VAD policy for quiet but representable speech.
- **2026-08-27 — Neural-TTS waveform integrity becomes non-empty + finite + non-silent before serialization.** Upstream CosyVoice evidence showed some requests can produce 0-second audio, while Hottop's local Qwen3-TTS and CosyVoice3 writers could still serialize a non-empty all-zero waveform as a successful WAV after the earlier NaN/Inf closure. PR #113 established a shared fail-closed contract: reject empty, non-finite or exact-all-zero samples before PCM/WAV creation. The silence gate is intentionally exact-zero only; broader loudness/VAD heuristics remain separate measured delivery concerns. This supersedes the weaker assumption that finite samples alone constitute a valid neural-TTS waveform.
- **2026-08-26 — Production repeatability becomes quality-contract-first; byte equality is scoped evidence.** Real 720p Odyssey runs #29/#32/#38 produced identical final bytes, while successful run #31 produced different shot/final hashes yet remained visually near-identical and passed the same seam/media/integrity gates. Hottop therefore defines repeatability by reproducible visual/audio/media/integrity contracts plus bound source/provenance/runtime identity. Byte equality remains useful additional evidence only for the exact observed scope. This supersedes any route-level claim of universal bitwise determinism and forbids weakening quality/throughput solely to preserve hashes.
- **2026-08-26 — Guaranteed local speech fallback operationalizes role and delivery when safe controls exist.** Production-smoke showed `speaker` and `delivery` were preserved in `hottop.video-plan.v1` but the eSpeak-family fallback still emitted identical voice/rate controls. The guaranteed fallback now gives a recurring character stable bounded deterministic pitch and maps delivery cues to bounded cadence around the configured base rate; production-smoke probes the same resolver before full cow/Odyssey production. This supersedes the weaker assumption that preserving role/delivery metadata alone makes fallback dialogue role-aware. It remains purposeful synthetic differentiation, not a neural/natural-acting or collision-free identity claim.
- **2026-08-26 — Mobile subtitle readability now includes line-break quality, not only safe-area/block-height gates.** Production-smoke #127 showed the short mixed CJK/Latin caption `用 InkClawAgent。` wrapped into a one-character `用` line despite passing existing bounds. The first single-line attempt passed CI but failed real smoke #130; the corrected policy shrinks a natural-width single-line label within the readable font floor before falling back to wrapping, and smoke #132 proved the sentence remains on one line. This supersedes the assumption that safe containment plus acceptable block height guarantees readable typography.
- **2026-08-26 — Qwen delivery control is checkpoint-capability gated, not inferred from the CustomVoice API surface.** Fresh upstream source review during PR #57 showed the current 0.6B CustomVoice checkpoint explicitly discards `instruct` while the current 1.7B checkpoint supports it. Hottop now reads the already-provisioned local checkpoint config and fail-closes the role-aware Production route on 0.6B instead of silently dropping `AudioCue.delivery`. The standalone adapter may still perform 0.6B basic synthesis when no instruction is requested. Preset-speaker commercial/publication rights remain a separate operator gate from Apache-2.0 repository/model metadata.
- **2026-08-26 — Mobile-first framing now requires readable principal-subject scale as well as placement.** Production-smoke #105 artifact inspection showed Odyssey key characters could satisfy vertical safe-area placement yet remain only about 12–15% of portrait frame height, while the cow flagship read around 21%. PR #53 added a backend-specific mobile readability contract and a portrait-only Odyssey focal correction, verified by CI #1540 and production-smoke #106. This supersedes the weaker assumption that fixing empty upper-canvas placement alone proves mobile framing quality; thresholds remain measured/style/backend specific rather than universal.
- **2026-08-25 — Deterministic story routing becomes fail-closed.** Direct production artifact inspection found a correct Odyssey plan/subtitle track could still render the cow/workroom world. PR #39 fixed output-workspace plan routing, PR #40 made missing plans fail closed, and PR #43 removed the residual blank/unknown-topic-to-cow fallback. Hottop now rejects unsupported deterministic stories rather than emitting a technically valid but semantically false MP4. Distinct story proofs must remain materially distinct visual worlds.
...
7. For new image/video generation, perform the fresh hotspot pass or supplied-hotspot mechanism analysis before generation.
